...
class Ailf_KR:
    ...
    def KRXStockInvest(self, i=None, long_period=200, short_period=30, back_shifting=0, decompose_type='stl', resid_transform=False, scb=(0.1,0.9)):
        ...
        print(f' - DOWN Case')
        print(f'   > Open[+1]-Close[0] = {int(down_dev1)}|(std={int(down_dev1_std)})')
        print(f'   > Open[+1]/Close[0] = {int(down_dev2)}|(std={int(down_dev2_std)})')
        print(f' - UP Case')
        print(f'   > Open[+1]-Close[0] = {int(up_dev1)}|(std={int(up_dev1_std)})')
        print(f'   > Open[+1]/Close[0] = {int(up_dev2)}|(std={int(up_dev2_std)})')
        print(f'   > Open Est.  : {df.Close[-1] + up_dev1}|(std={int(up_dev1_std)}) ~ {df.Close[-1] * up_dev2}|(std={int(up_dev2_std)})')
        print(f'   > (based-Open)  Buy  : <Open Price> + {int(min_open_dev1)}|(std={int(min_open_dev1_std)}) ~ <Open Price> * {int(min_open_dev2)}|(std={int(min_open_dev2_std)})')
        print(f'   > (based-Open)  Sell : <Open Price> + {int(max_open_dev1)}|(std={int(max_open_dev1_std)}) ~ <Open Price> * {int(max_open_dev2)}|(std={int(max_open_dev2_std)})')
        print(f'   > (based-Close) Buy  : <Close Price> + {int(min_close_dev1)}|(std={int(min_close_dev1_std)}) ~ <Close Price> * {int(min_close_dev2)}|(std={int(min_close_dev2_std)})')
        print(f'   > (based-Close) Sell : <Close Price> + {int(max_close_dev1)}|(std={int(max_close_dev1_std)}) ~ <Close Price> * {int(max_close_dev2)}|(std={int(max_close_dev2_std)})')
