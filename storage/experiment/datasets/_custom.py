...
class CustomDataset(Dataset):
    def __init__(self, options):
        ...
        if self.options.dataset_name == 'internal-univariate-linear-scalar':
            self.dataset = obj()
            self.dataset.xy = torch.arange(0*1000*2, 3*1000*2).view(3*1000,2).type(torch.FloatTensor)
            permutation = torch.randperm(3*1000)
            spliter = permutation.split([1000,1000,1000])
            ...
            self.train_dataset = obj()
            self.train_dataset.xy = torch.arange(0*1000*2, 1*1000*2).view(1000,2).type(torch.FloatTensor)
            self.train_dataset.x = self.train_dataset.xy[:, 0:1]
            self.train_dataset.y = self.train_dataset.xy[:, 1:2]
            
            self.validation_dataset = obj()
            self.validation_dataset.xy = torch.arange(1*1000*2, 2*1000*2).view(1000,2).type(torch.FloatTensor)
            self.validation_dataset.x = self.validation_dataset.xy[:, 0:1]
            self.validation_dataset.y = self.validation_dataset.xy[:, 1:2]

            self.test_dataset = obj()
            self.test_dataset.xy = torch.arange(2*1000*2, 3*1000*2).view(1000,2).type(torch.FloatTensor)
            ...
            self.dataset = obj()
            self.dataset.xy = torch.arange(0*1000*10, 3*1000*10).view(3*1000,10).type(torch.FloatTensor)
            permutation = torch.randperm(3*1000)
            spliter = permutation.split([1000,1000,1000])
            ...
            self.train_dataset = obj()
            self.train_dataset.xy = torch.arange(0*1000*10, 1*1000*10).view(1000,10).type(torch.FloatTensor)
            self.train_dataset.x = self.train_dataset.xy[:, 0:9]
            self.train_dataset.y = self.train_dataset.xy[:, 9:10]
            
            self.validation_dataset = obj()
            self.validation_dataset.xy = torch.arange(1*1000*10, 2*1000*10).view(1000,10).type(torch.FloatTensor)
            self.validation_dataset.x = self.validation_dataset.xy[:, 0:9]
            self.validation_dataset.y = self.validation_dataset.xy[:, 9:10]

            self.test_dataset = obj()
            self.test_dataset.xy = torch.arange(2*1000*10, 3*1000*10).view(1000,10).type(torch.FloatTensor)
